...
import { declareSimpleTests, hookInParallel } from "./helpers.js";
...
export function defineCloseTests() {
  ...
  it("should call the before hooks in parallel", async () => {
    await hookInParallel("before.find.cursor.close", ({ hookedCursor }) => hookedCursor.close());
  });

  it("should call the error hooks in parallel", async () => {
    await hookInParallel("after.find.cursor.close.error", async ({ hookedCursor, fakeCursor }) => {
